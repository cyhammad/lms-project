...
import { ArrowLeft, GraduationCap, Calendar, CheckCircle2, XCircle, Clock, TrendingUp } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { getStudentById } from '@/lib/student-storage';
...
export default function StudentAttendanceHistoryPage() {
  ...
  const studentAttendances = useMemo(() => {
    if (!studentId) return [];
    return getPersonAttendance(studentId, 'student');
  }, [studentId, getPersonAttendance]);

  const stats = useMemo(() => {
    if (!studentId) {
      return { total: 0, present: 0, absent: 0, late: 0, presentPercentage: 0 };
    }
    return getPersonAttendanceStats(studentId, 'student');
  }, [studentId, studentAttendances]);

  // Filter attendances by selected month/year
  const filteredAttendances = useMemo(() => {
    return studentAttendances.filter(attendance => {
      const date = new Date(attendance.date);
      return date.getMonth() + 1 === selectedMonth && date.getFullYear() === selectedYear;
    }).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [studentAttendances, selectedMonth, selectedYear]);
  ...
  useEffect(() => {
    if (studentId) {
      ...
      if (studentData) {
        ...
        router.push(ROUTES.ADMIN.ATTENDANCE.STUDENTS);
      }
      setLoading(false);
    }
  }, [studentId, router]);
  ...
  const displayStudentId = student.studentId || student.bFormCrc || 'N/A';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link href={ROUTES.ADMIN.ATTENDANCE.STUDENTS}>
          ...
          <div>
            ...
            <p className="text-slate-800 mt-1">{displayName} - {displayStudentId}</p>
          </div>
        </div>
      </div>
        ...
        <CardContent className="p-6">
          <div className="flex items-center gap-6">
            {student.studentPhoto ? (
              ...
              <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-slate-800 to-slate-800 flex items-center justify-center text-white font-bold text-2xl shadow-lg shadow-slate-700/20">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-slate-800">
                ...
                {studentSection && (
                  <div className="flex items-center gap-2">
                    <span>Section: {studentSection.name}</span>
                  </div>
                )}
              </div>
            </div>
